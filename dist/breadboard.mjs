...
export function boardEdges(parts){return[...internal,...parts.flatMap(p=>footprint(p.id,rowOf(p))),...(parts.some(p=>p.id==='amp')&&parts.some(p=>p.id==='speaker')?[{a:'speaker:+',b:'amp:+'},{a:'speaker:-',b:'amp:-'}]:[])];}
